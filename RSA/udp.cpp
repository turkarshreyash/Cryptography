...
void UDPHost::send(uint16_t size, char *message,uint8_t oct4, uint8_t oct3, uint8_t oct2, uint8_t oct1, uint16_t port){
    ...
    send(size, message, cliaddr_in);
}

void UDPHost::send(uint16_t size, char *message, sockaddr_in cliaddr_in){
    sendto(sockfd, message, size, 0, (struct sockaddr*)&cliaddr_in, sizeof(cliaddr_in));
}
